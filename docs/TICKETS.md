...
## T-030 [P1] สัญญา+คู่มือสำหรับทีม AI — topic ภาพดิบ และ device_status ต่อ Zero — todo
why:        D-024 ทำให้เกิดของใหม่ 2 อย่างที่ทีม AI ต้องทำตาม แต่ยังไม่มีที่ไหนเขียนไว้:
            (ก) topic สำหรับ Zero ส่งภาพดิบเข้า broker — ต้องไม่ชนกับ `evidence/` (คนละความหมาย
            คนละปริมาณ) และต้องเป็นรูปแบบที่ ingest เราไม่ sub
            (ข) 🔴 **โมเดล process เดียว publish แทนหลาย Zero ⇒ LWT บอกได้แค่ "โมเดลตาย"**
            แยกไม่ออกว่า Zero ตัวไหนหยุดส่ง — โมเดลต้อง publish `device_status OFFLINE` ต่อ Zero
            เองเมื่อเฟรมหยุดมา ไม่งั้น UI จะเห็นแค่ "ไม่ส่งข้อมูล" (stale) ซึ่งช้ากว่าและกำกวมกว่า
scope:      `src/contract/topics.ts` (เพิ่ม topic ภาพดิบ + comment ว่า ingest ห้าม sub) ·
            `docs/PUBLISHING-GUIDE.md` (หัวข้อใหม่สำหรับระดับตัวถูก: topic ภาพดิบ · QoS 0 ไม่ retain ·
            กติกา device_status ต่อ Zero · ขนาดภาพสูงสุด) · แนบตัวอย่าง payload จริง
done-when:  ส่งคู่มือฉบับใหม่ให้ทีม AI แล้ว + `verify-contract` ยังผ่านเท่าเดิม
note:       ⚠️ อย่าลืมว่าคู่มือฉบับก่อนหน้าส่งไปแล้ว — ต้องบอกให้ชัดว่า**เพิ่ม**อะไร ไม่ใช่ส่งทั้งฉบับ
            ให้เขาหาเอง (บทเรียนจาก D-016/D-022) ; ระบุ `message_size_limit` ที่ broker จะตั้ง
            (T-029) ให้เขารู้ก่อน ไม่งั้นภาพใหญ่จะหายเงียบ ๆ

## T-029 [P2] broker/ingest รองรับ topic ภาพดิบของระดับตัวถูก — todo
why:        D-024: Zero ส่งภาพเข้า broker เรา — **เป็น snap เป็นรอบ ๆ เหมือน evidence เดิม
            ไม่ใช่ทุกเฟรม** (ผู้ใช้ยืนยัน 2026-09-11 ; เรื่อง "ทุกเฟรม" ยังไม่ได้เคาะกับทีม AI
            ถือเป็นเรื่องตอน dev) ⇒ ปริมาณอยู่ในงบเดิมของ D-013 ไม่ต้องเร่งเรื่อง persistence
            ที่เหลือจริงคือ: topic ต้องไม่ถูก ingest จับเป็น invalid (กับดักเดิม T-011) + เพดานขนาดภาพ
scope:      - ยืนยันว่า topic ภาพดิบ (จาก T-030) **ไม่ match** subscription ของ ingest — เขียนเทส
              ยิงภาพเข้า topic นั้นแล้ว `ingest.invalid` ต้องไม่ขยับ
            - `deploy/mosquitto.conf`: ตั้ง `message_size_limit` (ของค้างจาก T-011)
            - `DEPLOYMENT.md`: เติมบรรทัดว่า "ระดับตัวถูก: โมเดลทีม AI ลงแยก ดูคู่มือของเขา"
              ไม่งั้นคนติดตั้งจะคิดว่าเสร็จแล้วทั้งที่ยังอ่านค่าไม่ได้
done-when:  ยิงภาพ 20 ใบเข้า topic ใหม่ → `ingest.invalid` = 0 · ภาพเกินเพดานถูกปฏิเสธ (ไม่หายเงียบ)
note:       ⚠️ ถ้าวันหนึ่งทีม AI ยืนยันว่าจะส่ง**ทุกเฟรม**จริง ต้องกลับมาเปิดเรื่อง QoS 0/ไม่ retain/
            persistence ใหม่ — ตอนนั้นปริมาณจะคนละโลกกับที่ D-013 คิดงบไว้
## T-028 [P2] วัดว่า Pi 5 เครื่องเดียวรับกล้องได้กี่ตัว (soak test) — todo
why:        ทีมอยากรู้ว่า "UI รับ edge ได้กี่ device" — แต่ **คำถามนั้นวัดผิดตัว**: ingest ฝั่งเรา
            รับ 26 เฟรม/วิ/จุดได้อยู่แล้วและ throttle ทิ้ง 89.5% (D-012) ; คอขวดใหม่คือ
            **Pi 5 ต้องรันโมเดลเองพร้อมกับ broker + ingest + Postgres + เว็บ + Chromium kiosk**
            บนเครื่องเดียว ⇒ สิ่งที่ต้องวัดคือ "รับกล้องได้กี่ตัวโดยที่ยังทำทุกอย่างพร้อมกันได้"
scope:      ต้องนิยาม workload ให้ชัดก่อนวัด (กี่จุด/กล้อง · ขนาดภาพ · ความถี่ที่ Zero ส่ง) แล้ววัด:
            - อัตราการอ่านจริงต่อกล้อง (วิ/เฟรม) เมื่อเพิ่มกล้องทีละตัวจนเริ่มตก
            - CPU · RAM · **อุณหภูมิ + สถานะ throttle** (`vcgencmd get_throttled`)
            - จอ kiosk ยังลื่นไหม (คนหน้างานเห็นอาการนี้ก่อนใครเพื่อน)
            - `ingest.invalid` ยัง 0 · DB เขียนทัน · SD โตวันละเท่าไหร่
done-when:  ได้ตัวเลข "กล้องสูงสุดต่อ Pi 5 หนึ่งเครื่อง" ที่มาจากการรันจริงต่อเนื่อง **≥3 ชั่วโมง**
            พร้อมกราฟ/ตารางว่าอะไรตกก่อนเมื่อเกินจำนวนนั้น
note:       🔴 **ต้อง soak test ไม่ใช่เทส 5 นาที** — Pi 5 รันโมเดลต่อเนื่องแล้วร้อนจนลดความเร็ว
            ลงเรื่อย ๆ เทสสั้นจะผ่านสวยแล้วพังตอนชั่วโมงที่ 3
            ; 🔴 **วัดปริมาณภาพที่ไหลผ่าน broker ด้วย** — D-013 ออกแบบให้ส่งภาพเฉพาะตอน
            `quality != OK` เพดาน 1 ภาพ/นาที/จุด (~72MB/วัน) ; ถ้า Zero ส่งทุกเฟรม
            ปริมาณจะคนละโลก และ broker เขียนลง SD ซ้ำเพราะเปิด persistence
            ; ผลของใบนี้คือตัวเลขที่ใช้ตอบลูกค้าว่าแพ็กเกจตัวถูกรับได้กี่จุด — เก็บให้อ้างอิงได้
...
## T-025 [P1] UI แจ้งเตือน — toast ขวาล่าง + สถานะค้างบนการ์ด — done
why:        ครึ่งที่คนเห็น ; ทีมขอ noti เด้งมุมขวาล่างแบบลื่น ๆ
scope:      component toast ใหม่ (ขวาล่าง, เลื่อนเข้า/ออก, respect `prefers-reduced-motion`) ·
            สถานะ `ALARM` บนการ์ด (rail + tag ตามภาษาเดิมของ D-019) · ตัวนับจุดผิดปกติที่หัวจอ
done-when:  ค่าออกนอกเกณฑ์ → toast เด้ง 1 ครั้ง ไม่ใช่ทุกเฟรม · toast หายเอง แต่การ์ดยังค้างสถานะ ·
            กลับเข้าเกณฑ์ → การ์ดกลับปกติ · เปิด 2 แท็บพร้อมกันเห็นตรงกัน
note:       🔴 **จอ kiosk ไม่มีคนกดปิด** — toast ต้องหายเอง (~10 วิ) และ**จำกัดจำนวนที่ซ้อนได้**
            (เช่น 3 อัน) ไม่งั้นวันที่หลายจุดเสียพร้อมกันจะบังจอทั้งหมด
            ; toast เป็นของชั่วคราว **ห้ามเป็นที่เดียวที่บอกว่าผิดปกติ** — คนเดินมาทีหลังต้องรู้ได้
            จากการ์ด ; ไม่ใส่เสียง (โรงงานเสียงดัง + จอไม่มีคนอยู่ประจำ)
            ; ห้ามใช้ emoji — ใช้ SVG ใน `Icons.tsx` (D-019)
files:      src/web/{useLiveData.ts,apiClient.ts,App.tsx,styles.css} ·
            src/web/components/{AlarmToasts.tsx (ใหม่),PointCard.tsx,Icons.tsx} · scripts/check-contrast.ts
done: 2026-09-11 **การตัดสินใจที่ต้องจด**: ALARM อยู่ในลำดับสถานะการ์ด**หลัง stale แต่ก่อน unreadable**
      — stale ต้องชนะเพราะ D-023 ให้ดู "ค่าเก่า" คู่กัน ; แต่ ALARM ต้องชนะ unreadable ไม่งั้น
      UNREADABLE 47% จะทำการ์ดกระพริบสลับ "ผิดปกติ/อ่านไม่ออก" ทั้งวันทั้งที่สถานะไม่ได้เปลี่ยน
      ; ALARM เป็นสถานะเดียวที่ "ดังกว่า" เพื่อน (rail 4px + tag ทึบ) โดยไม่เพิ่มสีใหม่ ใช้ --bad
      เหมือน unreadable แต่แยกออกจากไกลได้ด้วยความหนา/ความทึบ (D-019: 1 ของหลักต่อจอ)
      ; toast เด้งเฉพาะ →ALARM และ ALARM→OK — ไม่เด้งตอนเริ่มเฝ้า (null→OK) หรือถอนเกณฑ์ (→null)
      ; แตะ toast เพื่อปิดได้ แต่ไม่มีปุ่ม X แยก (ปุ่มบนจอที่ไม่มีใครกดคือของปลอม)
      ; verify ผ่าน DB + reload: การ์ด ALARM (tag ทึบ/rail 4px/ตัวเลขแดง) · หัวจอ "1 ผิดปกติ"
      อยู่ซ้ายของ "N/M จุดปกติ" · กลับเข้าเกณฑ์ → card-ok ตัวนับหาย · check-contrast ผ่าน
      (tag ทึบ 6.46/8.82:1) · type-check + build ผ่าน
      ⚠️ **toast เด้งสดยังไม่ได้เห็น** — browser pane ที่ซ่อนอยู่ตัดสาย EventSource หลัง `hello`
      (พิสูจน์แล้วว่า Vite proxy + backend ส่ง event ถูก: curl ผ่าน :5173 ได้ readings ปกติ)
      ; ฝากดูตอน deploy — เกณฑ์ 1–3 บน `pt-a-boiler-pressure` (สเกล 0–4) ทิ้งไว้บน dev
      ให้ mock สุ่มค่าเด้ง toast ให้ดูได้เลย
      🔴 **บทเรียนสภาพแวดล้อมเทส**: `bun --hot` reload ทุกครั้งที่แก้ไฟล์ (รวมไฟล์ฝั่งเว็บ) แล้ว
      **ล้างตัวนับ N ในหน่วยความจำ** — เทสที่ต้องยิงติดกัน N ครั้งจะไม่มีวันครบถ้าแก้โค้ดไปด้วย
      ต้องรัน `bun src/server/index.ts` ตรง ๆ (ไม่ --hot) ตอนเทส



## T-026 [P1] ฟอร์มตั้งเกณฑ์เตือนจาก UI — done
why:        เกณฑ์ต่างกันทุกจุดทุกโรงงาน ถ้าตั้งได้แค่ทาง SQL ก็ใช้งานจริงไม่ได้
            ; ฟอร์มตั้งค่าจุดวัดมีอยู่แล้วใน `PointDetail.tsx` — เติมช่องเข้าไป ไม่ต้องสร้างของใหม่
scope:      เติม 2 ช่องในฟอร์มเดิม (`d-cfg`) + ขยาย `pointConfigInput` ฝั่ง server ·
            **ต้องเขียนให้ชัดบนหน้าจอว่าคนละเรื่องกับสเกล** เพราะฟอร์มเดียวกันมี min/max อยู่แล้ว
            คนกรอกจะสับสนแน่ถ้าไม่บอก
done-when:  ตั้งเกณฑ์จากจอได้ · เว้นว่างได้ (= ไม่เตือนจุดนั้น) · ใส่ครึ่งเดียวไม่ได้ ·
            ค่าที่ตั้งมีผลทันทีโดยไม่ต้อง restart
note:       กติกาเดียวกับ min/max: ต้องมาคู่กันหรือเว้นว่างทั้งคู่ ; และ **ห้ามบังคับว่าเกณฑ์ต้อง
            อยู่ในช่วง min–max** — เข็มชี้เลยสุดสเกลเป็นเรื่องปกติ (คอมเมนต์ท้าย `contract/points.ts`)
files:      src/server/api/points.ts · src/web/{apiClient.ts,components/PointDetail.tsx,styles.css}
done: 2026-09-11 เติม 2 ช่องในฟอร์มเดิม + **ใส่หัวกลุ่มให้ทั้งสองคู่** ("สเกลของหน้าปัด" /
      "ช่วงที่ยอมรับได้") พร้อมคำอธิบาย 1 บรรทัดที่บอกว่าต่างกันยังไง — เดิม min/max ไม่มีหัวกลุ่ม
      ด้วยซ้ำ · ดึงตรรกะ "คู่/ตัวเลข/สูง>ต่ำ" เป็น `parsePair()` ใช้ร่วมกันทั้งสองกลุ่ม
      🔴 **จุดที่จับได้ก่อน commit**: server เดิมร่างเป็น `.default(null)` ซึ่งจะทำให้ client เก่า
      (kiosk cache JS) ที่ส่งฟอร์มมาโดยไม่มีฟิลด์ใหม่ **ล้างเกณฑ์ที่ตั้งไว้เงียบ ๆ** → เปลี่ยนเป็น
      `.optional()` = ไม่ส่งมา ไม่แตะ ; พิสูจน์ด้วย curl ว่าส่ง body ไม่มีฟิลด์แล้วค่าเดิมยังอยู่
      ; verify server 5 เคส (คู่ถูก · ครึ่งคู่ · สูง<ต่ำ · **นอกสเกลต้องรับ** · ไม่ส่งฟิลด์ต้องไม่ล้าง)
      + UI 4 เคส (แสดงค่าเดิมจาก DB · บันทึก 1/3 · error 2 แบบดักที่ client · เว้นว่างทั้งคู่ =
      null/null) ; type-check + build + check-contrast ผ่าน
      ⚠️ ยังไม่ได้ดูหน้าตาด้วยตา (browser pane วาดไม่ได้) — ฝากดูตอน deploy



## T-024 [P1] backend ของ alarm — เกณฑ์ + ตัดสินที่ ingest + ส่งขึ้นจอ — done
why:        ครึ่งที่ทำให้ทั้งฟีเจอร์เป็นจริง ; ตอบ **OPEN-6** ที่ค้างในสัญญามาตั้งแต่ต้น
            (ดู D-023 สำหรับเหตุผลทั้งหมด อย่าเล่าซ้ำที่นี่)
scope:      - `db/schema.ts` + migration: `alarm_low` / `alarm_high` (nullable, คู่กัน) และ
              `alarm_state` (`OK`/`ALARM`/null) + `alarm_since` บนตาราง `points`
            - `ingest/` : ประเมินหลัง throttle ตัดสินใจเก็บแล้ว → เปลี่ยนสถานะเมื่อ**เกินติดกัน
              N ครั้ง** (`ALARM_CONFIRM_READINGS`, default 3) → เขียน DB เฉพาะตอน**เปลี่ยน** →
              emit event ขึ้น SSE
            - `api/points.ts` : คืน 4 ฟิลด์ใหม่ใน `GET /api/points`
            - `.env.example` : เอกสารของค่าใหม่
done-when:  ยิงค่าเกินเกณฑ์ติดกัน 3 ครั้ง → `alarm_state` เปลี่ยนเป็น ALARM + มี event ออก SSE
            **ครั้งเดียว** · ยิงเกินต่ออีก 100 ครั้ง → ไม่มี event เพิ่ม · กลับเข้าเกณฑ์ 3 ครั้ง →
            กลับ OK + event เดียว · ยิง `UNREADABLE` แทรก → สถานะไม่เปลี่ยนและไม่นับ N
note:       🔴 **`UNREADABLE` ต้องไม่ประเมินและไม่รีเซ็ตตัวนับ** — "ตรวจไม่ได้ ≠ ของเสีย"
            (ARCHITECTURE) ; ผลข้างเคียงคือสถานะค้างที่ค่าล่าสุดถ้าอ่านไม่ออกยาว ๆ ซึ่งยอมรับ
            ตาม D-023 — UI ต้องดู "ค่าเก่า" คู่เสมอ
            ; จุดที่ไม่ได้ตั้งเกณฑ์ = ไม่ประเมินเลย (`alarm_state` เป็น null ไม่ใช่ OK)
            ; ค่าที่เป็นข้อความล้วนแปลงเป็นเลขไม่ได้ = ไม่ประเมิน (ต่อจาก T-023)
            ⚠️ เขียน DB เฉพาะตอนเปลี่ยนสถานะเท่านั้น — ถ้าเขียนทุกเฟรมจะกลายเป็นภาระ SD
            แบบเดียวกับที่ D-012 เพิ่งแก้ไป
progress:   2026-09-11 โค้ดครบทุกส่วน: schema 4 คอลัมน์ + migration `0003` · `ingest/alarm.ts`
            (pure function แยกไฟล์ให้เทสได้) · ต่อเข้า `handleFrame` โดยประเมิน**ทุก** reading
            ไม่ใช่เฉพาะที่เก็บ · เขียน DB + emit `alarm` เฉพาะตอนเปลี่ยน · SSE forward ·
            `GET /api/points` คืน 4 ฟิลด์ · `invalidatePointConfig()` ให้ PATCH มีผลเฟรมถัดไป
            ทันที (ไม่รอรอบล้าง 5 นาที) · `.env.example` · `bun run smoke-alarm`
            ✅ **smoke-alarm ผ่าน 18 ข้อ** (pure logic ไม่ต้องมี DB) — ครอบคลุม: เด้งครั้งเดียว
            ไม่ใช่ทุกเฟรม · ค่าวูบเดียวไม่เด้ง · แกว่งรอบขอบไม่เด้ง · UNREADABLE ไม่นับ/ไม่รีเซ็ต ·
            restart ไม่เด้งซ้ำ · ถอนเกณฑ์เคลียร์ทันที · ขอบเกณฑ์นับเป็น "ใน"
files:      src/db/{schema.ts,migrations/0003_*.sql} · src/server/ingest/{alarm.ts,index.ts} ·
            src/server/{events.ts,api/stream.ts,api/points.ts} · scripts/smoke-alarm.ts · .env.example
done: 2026-09-11 apply migration 0003 แล้ว ; **e2e บน MQTT จริงตรง done-when ทุกข้อ**: ปกติ 3 → OK
      (1 event) · เกิน 2 → ยัง OK · เกินครั้งที่ 3 → ALARM (2 events) · **เกินต่ออีก 30 → ยัง 2 events**
      · UNREADABLE ×3 → ไม่แตะ · กลับ 3 → OK (3 events) · `GET /api/points` คืน 4 ฟิลด์ ·
      `invalid` 0 · `alarm_transitions` 3 · SSE payload ครบ from/to/value/captured_at ·
      `alarm_since` ขยับทุกครั้งที่เปลี่ยน · `invalidatePointConfig` ผ่าน PATCH ทำให้เกณฑ์ที่ตั้ง
      หลังจุดถูกสร้างมีผลทันที ; ล้างข้อมูลทดสอบแล้ว · ปลดบล็อก T-025/T-026



## T-027 [P1] เคาะ: สถาปัตยกรรม 2 ระดับ (Zero ส่งภาพ + Pi 5 คิด / edge คิดเอง) — done
why:        Pi Zero 2 W รันโมเดลไม่ไหว (RAM 415MB) → ทีมจะเปลี่ยนให้ Zero **ส่งภาพอย่างเดียว**
            แล้วให้ **Pi 5 ที่รัน UI อยู่แล้วรันโมเดล 7SEG/WATER_METER เอง** เป็นแพ็กเกจตัวถูก
            ไว้สเกลยอดขาย ; ส่วนแบบเดิม (edge คิดเอง 1 edge 1 module) เป็น option ตัวแพง
            🔴 **ขัดกับ D-020 ที่เพิ่งเคาะไปว่า "ฝั่ง edge อยู่นอกขอบเขตเรา"** — พอโมเดลมาอยู่
            บนเครื่องเรา มันกลายเป็นของที่เราต้อง deploy/ดูแล/แก้เวลาพัง ; และ ROADMAP-PACKAGE
            ที่เขียนว่า "1 Pi : 1 โรงงาน" กลายเป็น **2 ระดับสินค้า** = เปลี่ยนปลายทางของแผนที่
scope:      เคาะให้ครบ 4 ข้อ แล้วจด DECISIONS.md
            1. **ภาพเดินทางจาก Zero → Pi 5 ยังไง** (คำถามหลัก) — MQTT เหมือน evidence เดิม /
               HTTP POST / stream / โฟลเดอร์แชร์ ; D-013 เลือก MQTT ตอนที่ภาพเป็น "ของนาน ๆ ที"
               ไม่ใช่ "ทุกเฟรม" เหตุผลเดิมอาจใช้ไม่ได้แล้ว
            2. **โมเดลบน Pi 5 คุยกับ server เรายังไง** — ⭐ ทางที่น่าจะดีที่สุดคือให้มัน
               **publish เข้า broker ตัวเดิมด้วยสัญญาเดิม** (`meter_frame`) ⇒ server เรา
               **แยกไม่ออกว่าเป็นระดับไหน** โค้ดที่ทำมาทั้งหมดใช้ต่อได้ไม่ต้องแก้
               (รักษา seam ที่ T-002/D-002 วางไว้) — เทียบกับทางอื่นก่อนตัดสิน
            3. **ใครเป็นเจ้าของโมเดลบนเครื่องเรา** — ทีม AI ส่ง artifact มาให้เรา deploy หรือเขา
               ลงเอง ; กระทบ `DEPLOYMENT.md` และ "โมเดลพักตอนตี 3 ใครตื่นมาแก้"
            4. **สองระดับใช้โค้ด/เอกสารชุดเดียวกันไหม** หรือแตกเป็นคนละ profile
done-when:  จด DECISIONS.md ครบ 4 ข้อ + อัปเดต `ROADMAP-PACKAGE.md` (ปลายทางเปลี่ยน) +
            อัปเดตส่วนที่ D-020 พูดถึงขอบเขต edge + รู้ว่าต้องเปิดใบทำจริงกี่ใบ
note:       ⚠️ **อย่าเพิ่งไปเทส performance (T-028) ก่อนเคาะข้อ 1** — ถ้ายังไม่รู้ว่าภาพส่งด้วย
            วิธีไหน เทสไปก็วัดของที่อาจไม่ได้ใช้
            ; ⚠️ ระวังผลกับ `docs/PUBLISHING-GUIDE.md` — ถ้าโมเดลบน Pi 5 publish สัญญาเดิม
            คู่มือนั้นยังใช้ได้ทั้งฉบับ ซึ่งเป็นเหตุผลสนับสนุนทางที่ 2 อีกข้อ
done: 2026-09-11 ผู้ใช้เคาะครบ 4 ข้อ → **D-024**: (1) ภาพดิบผ่าน MQTT พอร์ตเดิม (2) โมเดลบน Pi 5
      publish สัญญาเดิมเข้า broker เดิม (3) ทีม AI ลงเอง (4) แยก profile แค่อยู่ร่วมเครื่อง
      ; **ข้อสรุปสำคัญ**: ข้อ 2+3+4 รวมกันทำให้ D-020 "edge นอกขอบเขต" **ยังจริง** — ขอบเขตแค่
      ย้ายจาก "คนละเครื่อง" เป็น "คนละ process" ไม่ต้องรื้อ T-015/T-017/T-018
      ; สิ่งที่เปลี่ยนจริงสำหรับเรามี 3 เรื่องและแตกเป็นใบทำจริงแล้ว: ภาพดิบทุกเฟรมผ่าน broker
      (T-029) · LWT ตัวเดียวแยก Zero ที่ตายไม่ออก (T-030) · ทรัพยากรบน Pi 5 ถูกแย่ง (T-028)
      ; อัปเดต ROADMAP-PACKAGE (ปลายทาง 2 ระดับ + Out of scope) แล้ว
