...
## T-029 [P1] broker/ingest รองรับภาพดิบทุกเฟรมโดยไม่พัง — todo
why:        ระดับตัวถูก (D-024) ภาพดิบทุกเฟรมจะวิ่งผ่าน broker เรา — คนละโลกกับ evidence ของ
            D-013 (1 ภาพ/นาที/จุด เฉพาะตอนอ่านไม่ออก) ; ถ้าไม่ทำอะไร: (1) ingest sub `meter/+/+`
            อาจจับภาพเข้า parser แล้วนับเป็น invalid (กับดักเดิม T-011) (2) broker persist ลง SD
            ทุกเฟรม (3) ภาพใหญ่ไม่มีเพดาน
scope:      - ยืนยันว่า topic ภาพดิบ (จาก T-030) **ไม่ match** subscription ของ ingest — เขียนเทส
            ...
            - `deploy/mosquitto.conf`: ตั้ง `message_size_limit` (ของค้างจาก T-011) และเช็คว่า
              QoS 0 ไม่ retain ไม่โดน persistence จริง (`persistence true` อยู่ — ต้องพิสูจน์ว่า
              เขียน SD เฉพาะ retained/QoS≥1 ที่ค้าง)
...
done-when:  ยิงภาพดิบ 100 เฟรมเข้า topic ใหม่ → `ingest.invalid` = 0 · ไฟล์ persistence ของ
            broker ไม่โต · ภาพเกินเพดานถูกปฏิเสธ (ไม่ใช่หายเงียบ)
note:       เอาของค้าง T-011 (`message_size_limit`) มาปิดในใบนี้เลย ไม่ต้องแยก
...
## T-024 [P1] backend ของ alarm — เกณฑ์ + ตัดสินที่ ingest + ส่งขึ้นจอ — todo
