...
## T-021 [P2] ปุ่มหลักในฟอร์มไม่เคยได้สีส้มจริง — specificity ชนกัน — todo
why:        เจอระหว่างทำ T-019 (นอก scope ใบนั้น จึงเปิดใบใหม่) — `.d-cfg-save` ตั้ง
            `background: var(--primary)` ไว้ แต่ `.d-cfg-actions button` ที่อยู่ในไฟล์เดียวกัน
            มี specificity สูงกว่า **(0,1,1) > (0,1,0)** จึงชนะเสมอไม่ว่าเขียนก่อนหรือหลัง
            ⇒ ปุ่ม "บันทึก" ของฟอร์มตั้งค่าจุดวัดได้พื้น `--panel` กับตัวอักษร `--text`
            **ไม่เคยเป็นปุ่มหลักสีส้มอย่างที่ตั้งใจเลยตั้งแต่แรก** ; ในธีมมืดยิ่งแย่เพราะ
            `--panel` เกือบขาวทับกับตัวอักษรอ่อน = คู่สีที่ไม่ผ่าน AA
scope:      `web/styles.css` — `.d-cfg-save` (บรรทัด ~598) เขียนใหม่เป็น
            `.d-cfg-actions button.d-cfg-save` ให้ specificity (0,2,1) ชนะ ;
            ตรวจ `.d-calib-primary` vs `.d-calib-row button` ว่าโดนแบบเดียวกันไหม ;
            ไล่ดูคู่อื่นที่เข้าแพตเทิร์น `.x button {}` + `.x-something {}` ทั้งไฟล์
done-when:  เปิดฟอร์มตั้งค่าจุดวัดแล้วปุ่มบันทึกเป็นสีส้มจริง **ทั้งธีมสว่างและมืด** ·
            คู่สีตัวอักษร-พื้นของปุ่มผ่าน WCAG AA (คำนวณจริง ไม่กะด้วยตา)
note:       ✅ แบบที่เขียนถูกอยู่แล้วในไฟล์คือ `.theme-toggle button.on` — ใช้เป็นต้นแบบได้
            ; T-019 แก้ของตัวเองไปแล้วด้วยวิธีนี้ (`.dev-rename button.dev-rename-save`)
            ⚠️ **อย่าแก้ด้วย `!important`** — จะกลายเป็นหนี้ที่ใบถัดไปต้องสู้ต่อ
            ⚠️ ตรวจด้วยตาบนเบราว์เซอร์จริง — `getComputedStyle` ผ่าน browser tool ของ AI
            คืนค่าค้างได้ (เจอมาแล้วตอน T-019: รายงานสีธีมสว่างทั้งที่ root เป็นธีมมืด)
