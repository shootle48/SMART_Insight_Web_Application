...
## verify: ยืนยันด้วยตาบนเครื่องจริงครบแล้ว — ปิดข้อค้างของ 3 ใบพร้อมกัน  🟢
- ผู้ใช้ deploy แล้วดูเองบนเครื่อง 2026-09-10: **กราฟจุด 7SEG ขึ้นจริง** (T-023) ·
  **ปุ่ม "บันทึก" เป็นสีส้มทั้งธีมสว่างและมืด** (T-019/T-021)
- ทั้ง 3 ใบก่อนหน้านี้ verify ได้แค่ระดับ DOM/ตัวเลข เพราะ browser pane ของ AI มี
  `innerWidth/innerHeight = 0` + `visibility: hidden` ⇒ screenshot เป็นจอเปล่าตลอด
  ; **บทเรียน: งานที่ตัดสินด้วยสายตาต้องให้คนดู อย่าเขียนว่า "เสร็จ" จากค่า computed อย่างเดียว**
- `origin/main` = local = `2cc9f79` · ไม่มี commit แตะ `src/` ค้าง ⇒ **ไม่เหลือหนี้ deploy**
  และไม่เหลือของที่ "แก้แล้วแต่ยังไม่พิสูจน์บนเครื่องจริง"

---

## T-023: กราฟย้อนหลังไม่ขึ้นสำหรับ 7SEG/WATER_METER — ค่าเป็นตัวเลขแต่อยู่ผิดช่อง  🟡
- **อาการที่ผู้ใช้เจอบนเครื่องจริง**: ค่าปัจจุบันขึ้นปกติ แต่กราฟขึ้นข้อความ "ยังไม่มีข้อมูลพอ
  วาดกราฟในช่วงนี้" เฉพาะ SEVEN_SEGMENT กับ WATER_METER ; GAUGE ปกติทุกจุด
- **ต้นตอ**: สองชนิดนี้ edge ส่งเลขนับมาในช่อง `value_text` (ตรงกับ D-016) แต่ history รวมค่า
  ด้วย `avg(value_num)` → null ทุก bucket → `HistoryChart` เข้าเงื่อนไข `withValue.length < 2`
  แล้ว return ข้อความแทนการวาด SVG ; ต้นตอจริงอยู่ที่ edge ส่งผิดช่อง **แต่ edge อยู่นอก
  ขอบเขตเรา (D-020)** จึงรับมือฝั่งเรา
- แก้ 2 ที่ตามที่ผู้ใช้เคาะ (ทาง c): **SQL ของ history** cast ข้อความที่หน้าตาเป็นตัวเลขมารวมด้วย
  → **ข้อมูลเก่าที่เก็บไว้แล้วขึ้นกราฟทันที ไม่ต้องรอข้อมูลใหม่** · **ingest** เติม `value_num`
  ตอนรับเข้า → **ได้ deadband throttle กลับมาด้วย** ซึ่งจุดพวกนี้ไม่เคยโดนบีบเลยเพราะ deadband
  เทียบด้วยตัวเลข (เก็บทุกเฟรมเท่าที่เพดานอัตราอนุญาตทั้งที่ค่าไม่ขยับ)
- `HistoryChart` แยกข้อความเป็น 3 กรณีให้ตรงความจริง — ของเดิมพูดว่า "ไม่มีข้อมูลพอ" เหมือนกันหมด
  ซึ่งทำให้ไปตามหาสาเหตุผิดทาง ; และเอา `last_text` ที่ประกาศไว้ใน type แต่**ไม่มีใครใช้เลย**มาใช้จริง
- verify ด้วยข้อมูลที่ยิงผ่าน MQTT จริง: `"['9dot875']"` → เก็บเป็น `9.875` + เติม `value_num` ให้ ·
  `"ERR"` ยังเป็นข้อความ `value_num` ว่าง **ไม่ได้เลขมั่ว** · `invalid` คง 0 · history คืน
  `avg_value` เป็นเลขจริง (1207/1214/1221/1228/1242) · SVG วาดติดบนหน้าเว็บ ; ล้างข้อมูลทดสอบแล้ว
- ⚠️ **regex ฝั่ง SQL กับฝั่ง TS ต้องตรงกันเป๊ะ** ไม่งั้นข้อมูลก่อน/หลัง deploy ถูกนับคนละแบบ
  แล้วกราฟจะมีรอยต่อแปลกตรงวันที่ deploy — เขียนเตือนไว้ในทั้งสองไฟล์
- 📌 **ระหว่างทางพบว่า browser pane ของ AI มี `innerWidth/innerHeight = 0` และ
  `visibility: hidden`** — นั่นคือสาเหตุที่ screenshot ออกมาเป็นจอเปล่าและ geometry อ่านได้ 0
  มาตลอดทั้ง session (ไม่ใช่บั๊กของแอป) ; ค่าที่เชื่อได้จาก pane นี้คือโครง DOM เท่านั้น

---

## tooling: `bun run check-contrast` — ตรวจคู่สีอัตโนมัติ (เจอของพัง 4 คู่ตั้งแต่รันแรก)  🟢
- ผู้ใช้เคาะให้เอาสคริปต์ที่เขียนตอน T-021 เข้า repo — เหตุผล: นี่เป็นบั๊ก contrast **รอบที่ 3**
  (D-014 audit → D-015 ธีมมืด → T-021 ปุ่มหลัก) ทุกรอบเจอเพราะบังเอิญไปตรวจ ไม่ใช่เพราะมีอะไรเตือน
  ; และกะด้วยตาจับไม่ได้จริง — ส้มสดบนขาวดูคอนทราสต์ดีมากแต่ได้แค่ 2.86:1
- **อ่านค่า token จาก `styles.css` จริงเสมอ ไม่พิมพ์ค่าสีซ้ำลงในสคริปต์** — ถ้าพิมพ์ซ้ำ วันที่มีคน
  แก้ token แล้วลืมแก้สคริปต์จะได้ผลลวงว่ายังผ่าน ซึ่งแย่กว่าไม่มีสคริปต์เลย
- 🔴 **รันครั้งแรกเจอของที่พังอยู่ก่อนแล้ว 4 คู่** → เปิด **T-022** ; ต้นตอคือ D-014/D-015
  คำนวณเทียบกับ `--bg` (พื้นหน้า) แต่ไม่เคยเทียบกับ `--surface` (พื้นการ์ด) ซึ่งเข้มกว่า —
  **ของบนจอส่วนใหญ่คือการ์ด** เกณฑ์ที่ผ่านจึงผ่านผิดที่มาตลอด
- ใส่รายการ `KNOWN_FAIL` เพราะถ้าปล่อยให้สคริปต์แดงค้าง จะไม่มีใครรันมันอีก แล้วของใหม่ที่พัง
  จะหลุดตามไปด้วย ; แบบนี้ exit 0 เมื่อไม่มี**ของใหม่**พัง แต่ยังพิมพ์เตือนของเก่าทุกครั้ง
  และขึ้น 🎉 บอกให้ลบออกจากรายการเมื่อคู่ไหนถูกแก้แล้ว

---

## T-021: ปุ่มหลักไม่เคยเป็นสีส้มจริง — แก้ specificity + แยก token ให้ผ่าน AA  🟡
- **สาเหตุ**: `.d-cfg-save` (0,1,0) แพ้ `.d-cfg-actions button` (0,1,1) เสมอไม่ว่าเขียนก่อนหลัง
  ⇒ ปุ่มบันทึกได้พื้น `--panel` ; ที่ไม่มีใครจับได้มาก่อนเพราะ `:hover:not(:disabled)` ดัน
  specificity เป็น (0,3,0) จนชนะ **ปุ่มจึงเทาตอนปกติแต่ส้มตอน hover** ดูเหมือน hover effect ที่ตั้งใจ
- **กวาดทั้งไฟล์แล้วพังจุดเดียวจริง** — `.d-calib-primary`/`.d-calib-ghost` ปลอดภัยเพราะ
  `.d-calib-row button` ไม่ได้ตั้ง background/color/border ทับ · `.err button` ไม่มีคลาสเดี่ยวมาชน
  · `.theme-toggle button.on` เขียน selector ถูกอยู่แล้ว (ใช้เป็นต้นแบบ)
- 🔴 **แก้ specificity อย่างเดียวไม่พอ** — คำนวณจริงแล้ว `--primary` + ตัวอักษรขาว = **2.86:1**
  (สว่าง) / **2.82:1** (มืด) ตกเกณฑ์ AA 4.5:1 ทั้งคู่ ⇒ ถ้าแก้ให้เป็นสีส้มตามที่ตั้งใจไว้แต่แรก
  จะได้ปุ่มที่อ่านยากกว่าเดิม → แยก token พื้นปุ่มออกจากสีแบรนด์ (**D-021**)
- ธีมมืดต้องกลับด้านเป็น "ส้มสด + ตัวอักษรเข้ม" เพราะทางอื่นตันหมด: ส้มเข้มแยกจากพื้นการ์ดมืด
  ได้แค่ 2.21:1 (ตก 1.4.11) ส่วนส้มที่สว่างพอจะแยกออกจากพื้นมืดกลับรับตัวอักษรขาวไม่ไหว
- verify: เขียนสคริปต์คำนวณ contrast ที่ **อ่านค่า token จาก `styles.css` จริง** (ไม่พิมพ์ค่าซ้ำ
  มาเช็ค ไม่งั้นแก้ token แล้วลืมแก้สคริปต์จะได้ผลลวงว่าผ่าน) — ผ่านครบ 10 คู่ ; ตรวจ DOM บนหน้า
  ที่โหลดใหม่ทั้งสองธีม ได้ค่าตรงกับ token ทุกตัว ; `type-check` + `build` ผ่าน
- ⚠️ **ยืนยันด้วย DOM ไม่ใช่ภาพ** — browser pane ไม่ยอมวาด (จอเปล่า) และ `getComputedStyle`
  คืนค่าค้างหลังสลับธีมตอนรัน (รายงานค่าธีมเก่าทั้งที่ token อัปเดตแล้ว) ต้องโหลดหน้าใหม่ถึงอ่านถูก

---

## T-019: ตั้งชื่อเครื่อง edge จากจอได้แล้ว  🟡
- **ทำไม**: `ingest` สร้างแถว device เองตอนเจอ `device_id` แปลกหน้าโดยไม่ใส่ `label` →
  จอขึ้นแต่ id ดิบที่คนหน้างานไม่รู้ว่าตู้ไหน ; เดิมตั้งได้ทางเดียวคือ `db:seed` ซึ่งใช้ที่
  โรงงานลูกค้าไม่ได้ (เป็นก้อนหนึ่งของปลายทาง "ติดตั้งโรงงานที่ 2 โดยไม่แก้โค้ด")
- `PATCH /api/devices/:deviceId` ([devices.ts](src/server/api/devices.ts)) — **รับแค่ `label`
  อย่างเดียวโดยตั้งใจ** ; ฟิลด์อื่นในตารางเป็นของที่ edge รายงานเข้ามา ถ้าเปิดให้คนแก้จะกลายเป็น
  ข้อมูลปลอมที่ ingest เขียนทับทีหลังอยู่ดี
- UI: แตะที่**บรรทัดชื่อ**ใน DeviceBar → กลายเป็นฟอร์มในตัว ; **ครอบเฉพาะบรรทัดชื่อ ไม่ใช่ทั้งบล็อก**
  เพราะบล็อกกว้าง 260px+ ถ้ากดได้ทั้งใบคนหน้างานจะเผลอแตะบ่อยเกินจำเป็น (จอนี้ยังไม่มี auth — D-020)
  ; ไม่เพิ่มปุ่มดินสอถาวรเพราะการตั้งชื่อเกิดครั้งเดียวตอนติดตั้ง ของที่ใช้ปีละครั้งไม่ควรกินที่บนจอ
  ที่คนดูทุกวัน
- **แก้บั๊กพ่วง**: เดิม `.dev-name` fallback เป็น `device_id` ทั้งที่บรรทัดบนโชว์ id อยู่แล้ว →
  เครื่องที่ยังไม่ตั้งชื่อขึ้น `EDGE-01` กับ `edge-01` ซ้อนกัน ต่างแค่ตัวพิมพ์ ดูเหมือนของเสีย
  → เปลี่ยนเป็น **"ยังไม่ตั้งชื่อ"** แบบจางไม่หนา ซึ่งบอกใบ้ไปในตัวว่าตรงนี้แตะได้
- verify: `type-check` + `build` ผ่าน ; ทดสอบด้วย**เครื่องที่ ingest สร้างเองจริง** — publish
  `meter/edge-t019/meter_frame` เข้า broker แล้วแถวโผล่พร้อม `label: null` → ตั้งชื่อไทยจากจอ →
  ขึ้นทันทีไม่ต้อง reload → ยืนยันลง DB ; เคสพลาดครบ (ชื่อว่าง 400 · เกิน 80 ตัว 400 · เครื่อง
  ไม่มีจริง 404) ; ล้างเครื่องทดสอบออกจาก DB + retained บน broker แล้ว
- 🔴 **ยังไม่ได้ยืนยันด้วยตาในธีมมืด** — browser pane ไม่ยอมวาดช่วงท้าย (จอเปล่า) และ
  `getComputedStyle` คืนค่าค้าง (รายงานสีธีมสว่างทั้งที่ root เป็นธีมมืด) จึงเชื่อไม่ได้
- **เจอของนอก scope → เปิด T-021**: `.d-cfg-save` ของฟอร์มจุดวัดตั้ง `background: var(--primary)`
  ไว้ แต่ `.d-cfg-actions button` specificity สูงกว่า **(0,1,1) > (0,1,0)** จึงชนะเสมอ ⇒
  **ปุ่มหลักในแอปไม่เคยเป็นสีส้มจริงเลยตั้งแต่แรก** ; ของ T-019 เองแก้แล้วด้วย
  `.dev-rename button.dev-rename-save` (0,2,1) ตามแบบ `.theme-toggle button.on` ที่เขียนถูกอยู่แล้ว

---

## verify: deploy ขึ้น Pi แล้ว — บั๊กภาพ evidence ช้า 1 เฟรมหายจริงบนเครื่อง  🟢
- ผู้ใช้ deploy เองและยืนยัน 2026-09-09 ว่า `7b465c1` (endpoint ไม่เคยอ่าน `?f=`) ใช้ได้ปกติแล้ว
  บนเครื่องจริง — เดิมแก้บนเครื่อง dev อย่างเดียว ซึ่งตาม `WORKFLOW.md` ไม่นับว่าเสร็จ
- **ปิดหนี้ deploy ที่ค้างมาตั้งแต่ `af3ffa6`** — `origin/main` = `f09a58c` ซึ่งใหม่กว่า `104162f`
  จึงมีครบทั้ง fix ภาพ evidence · redesign UI (D-019) · polish ฟอนต์ไทย · T-013/T-014 calibrate
  ; ไม่มีโค้ดที่รันจริงค้างบนเครื่อง dev อีกแล้ว (6 commit ที่ล้ำ origin เป็น docs/tooling ล้วน)
- ผลต่อลำดับงาน: เกณฑ์ข้อ 1 ของ `WORKFLOW.md` §0 ("หนี้ที่ดอกเบี้ยเดินอยู่") ว่างลง →
  **T-015 ขึ้นเป็นใบแรกที่ควรหยิบ** (อัปเดตใน HANDOFF แล้ว)
- **T-014 ผ่านเงื่อนไข "ทดสอบบน Pi" ฝั่งเราครบแล้ว** แต่ยังปิดใบไม่ได้ — รอทีม AI เขียนฝั่ง edge
  (sub command + ตอบ evidence `kind=CALIBRATION` + apply config) ซึ่งอยู่นอกขอบเขตเรา
- ✅ **ยืนยัน hash จากเครื่องจริงแล้ว**: Pi อยู่ที่ `f09a58c (HEAD -> main, origin/main, origin/HEAD)`
  = ตรงกับ `origin/main` เป๊ะ ไม่มี local change ค้างบนเครื่อง (ไม่ใช่การอนุมานจากคำบอกเล่าแล้ว)
