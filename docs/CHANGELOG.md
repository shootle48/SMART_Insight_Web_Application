...
## T-026 ปรับ: ตัดคำอธิบายใต้หัวกลุ่มออก — ผู้ใช้บอกว่างุนงงกว่าไม่มี  🟢
- ผู้ใช้ดูหน้าจริงแล้ว (ภาพยืนยันว่าฟอร์ม/ปุ่มส้มเข้มขึ้นถูก) บอกให้เหลือแค่หัวกลุ่ม "สเกลของหน้าปัด" /
  "ช่วงที่ยอมรับได้" — คำอธิบาย 1 บรรทัดที่ผมใส่เพื่อ "บอกว่าต่างกันยังไง" กลับทำให้ตาลาย
- **บทเรียน**: หัวกลุ่มที่ตั้งชื่อดีพอทำหน้าที่แทนคำอธิบายได้ — ถ้าต้องอธิบายใต้หัว แปลว่าหัวยังไม่ชัด
  หรือคนอ่านไม่ได้ต้องการรู้ตรงนั้น ; ตัด `.d-cfg-hint` ออกทั้ง TSX/CSS

---

## docs: `MQTT-CHECKLIST.md` — ทีมถาม "ข้อมูลส่งมาไหม" ตอบยังไง ทีละ topic  🟢
- ผู้ใช้ขอ checklist ตรวจข้อมูล MQTT ทุก topic ไว้ตอบทีม — เอา topic จาก `contract/topics.ts` ซึ่งเป็น
  ต้นทางจริง (6 topic + ช่องว่างสำหรับภาพดิบระดับตัวถูกที่รอ T-030) ไม่พิมพ์เอง
- **โครง 2 ชั้นทุก topic**: (1) ผ่าน broker ไหม → `mosquitto_sub` · (2) ถึงระบบเราไหม → health/DB/ไฟล์
  — ผ่านชั้น 1 แต่ไม่ผ่านชั้น 2 = ของเราพัง ; ไม่ผ่านชั้น 1 = ฝั่งเขายังไม่ส่งหรือส่งผิด topic
  แยกให้ออกตั้งแต่แรกว่าจะไปแก้ที่ใคร
- ทิป `-F '%I %t (%l bytes)'` แทน `-v` — ภาพเป็นไบต์ดิบ `-v` จะพ่นขยะเต็มจอ ; และย้ำว่า `meter/#`
  ปลอดภัยสำหรับคนดูแต่ ingest ห้าม sub (กับดัก T-011)
- **เขียนตามความจริงที่ไม่สวย**: health ยังไม่มีตัวเลข evidence (`evidenceIngestStats()` ค้างมาตั้งแต่
  T-011) ต้องดูไฟล์บนดิสก์แทน — ไม่แก้ในรอบนี้ แค่จดให้คนอ่านไม่ไปหาสิ่งที่ไม่มี
- ใส่เข้า reader (tier "เปิดเมื่อมีคำถาม") · `CLAUDE.md` ชี้ถึง · `DEPLOYMENT.md` เปลี่ยนจากเล่าซ้ำ
  เป็นชี้ไป (เรื่องเดียวอยู่ที่เดียว)

---

## T-025: UI แจ้งเตือน — toast ขวาล่าง + การ์ดค้างสถานะ + ตัวนับหัวจอ  🟡
- **ลำดับสถานะการ์ด**: ALARM อยู่หลัง stale แต่**ก่อน unreadable** — stale ต้องชนะ (D-023 ให้ดู
  "ค่าเก่า" คู่กัน) แต่ ALARM ต้องชนะ unreadable ไม่งั้น UNREADABLE 47% จะทำการ์ดกระพริบสลับทั้งวัน
- **ALARM ดังกว่าเพื่อนโดยไม่เพิ่มสี**: rail 4px (ปกติ 2px) + tag พื้นทึบ `--bad` ตัวอักษร `--on-banner`
  — แยกจาก unreadable ที่ใช้ `--bad` เหมือนกันได้จากระยะไกลด้วยความหนา/ความทึบ ไม่ต้องอ่านตัวหนังสือ
- **toast** (`AlarmToasts.tsx`): ขวาล่าง ซ้อน ≤3 หายเอง 10 วิ แตะเพื่อปิด (ไม่มีปุ่ม X แยก) ·
  เด้งเฉพาะ →ALARM และ ALARM→OK ไม่เด้งตอนเริ่มเฝ้า/ถอนเกณฑ์ · ไม่มีเสียง (Chromium บน kiosk
  ต้องขอ permission ซึ่งไม่มีใครกด) · `prefers-reduced-motion` ปิด animation · ไอคอน SVG ใหม่ 2 ตัว
- **หัวจอ**: "N ผิดปกติ" โผล่เฉพาะเมื่อมี วาง**ซ้าย**ของ "N/M จุดปกติ" ให้สายตาเจอก่อน
- verify: การ์ด ALARM/กลับ OK ผ่าน reload จาก DB · หัวจอนับถูก · check-contrast เพิ่มคู่ tag ทึบ
  (6.46/8.82:1) · type-check + build
- ⚠️ **toast เด้งสดยังไม่ได้เห็น** — browser pane ตัดสาย EventSource หลัง `hello` ; พิสูจน์แล้วว่า
  ไม่ใช่ Vite proxy (curl ผ่าน :5173 ได้ `readings`) ; ฝากดูตอน deploy
- 🔴 **บทเรียน**: `bun --hot` ล้างตัวนับ N ทุกครั้งที่แก้ไฟล์ — เทสที่ต้องยิงติดกัน N ครั้งต้องรัน server
  ตรง ๆ ไม่ใช่ผ่าน dev:api ; เสียเวลาไปรอบหนึ่งกว่าจะรู้

---

## T-026: ฟอร์มตั้งเกณฑ์เตือนจาก UI — เติมในฟอร์มเดิม + หัวกลุ่มกันสับสนกับสเกล  🟡
- เติม 2 ช่องในฟอร์มตั้งค่าจุดวัดเดิม (`PointDetail.tsx`) ไม่สร้างฟอร์มใหม่ · **ใส่หัวกลุ่มให้ทั้งสองคู่**
  ("สเกลของหน้าปัด" / "ช่วงที่ยอมรับได้") พร้อมคำอธิบายที่บอกว่า*ต่างกันยังไง* ไม่ใช่แค่บอกว่าคืออะไร
  — เดิม min/max ไม่มีหัวกลุ่มด้วยซ้ำ คนกรอกจะเอาเกณฑ์ไปใส่ช่องสเกลแน่
- ดึงตรรกะ "คู่กัน/ตัวเลข/สูง>ต่ำ" เป็น `parsePair()` ใช้ร่วมกันทั้งสองกลุ่ม — เขียนซ้ำแล้วเพี้ยนคือ
  บั๊กที่ยากจะจับเพราะสองกลุ่มหน้าตาเหมือนกัน
- 🔴 **จับได้ก่อน commit**: server ร่างแรกใช้ `.default(null)` → client เก่าที่ยังไม่รู้จักฟิลด์ใหม่
  (kiosk cache JS ไว้) ส่งฟอร์มมาจะ**ล้างเกณฑ์ที่ตั้งไว้เงียบ ๆ** ; เปลี่ยนเป็น `.optional()` =
  ไม่ส่งมา ไม่แตะ (drizzle `.set()` ข้าม key ที่ undefined) พิสูจน์ด้วย curl แล้ว
- **ตั้งใจไม่บังคับให้เกณฑ์อยู่ในสเกล** — เข็มชี้เลยสุดสเกลเป็นเรื่องปกติ (comment ท้าย `contract/points.ts`)
  ทดสอบแล้วว่า `-5/99` บนสเกล 0–4 ถูกรับ
- verify: server 5 เคส + UI 4 เคส (แสดงค่าเดิม · บันทึก · error ดักที่ client · เว้นว่าง = ยกเลิกเตือน)
  ; type-check + build + check-contrast ผ่าน ; ⚠️ หน้าตายังไม่ได้ดูด้วยตา
...
- ลบ `bboxSchema` + `sevenSegmentFixtureSchema` และยุบ `pointFixtureSchema` จาก
  ...
  อ่านด้วย**โมเดล** จึงไม่มีเรขาคณิตในภาพให้สอบเทียบ ; **ไม่ใช่ "ยังไม่ได้ทำ" แต่เป็น "ไม่มีอะไรให้ทำ"**
- ตัด union ฝั่งเว็บที่สะท้อนโครงเดิมไว้ (`apiClient.ts`) + แก้คอมเมนต์ที่ยังอ้าง bbox ใน `db/schema.ts`
- 📌 **note ของใบนี้เขียนไว้ผิด**: `pt-a-run-lamp` ไม่อยู่ใน `dev-inventory` แล้ว (D-016 ลบไป
  ตั้งแต่ 2026-09-03) — การ์ดที่เห็นบนจอ dev มาจาก**แถวเก่าใน DB** ไม่ใช่โค้ด
- 📌 **`PUBLISHING-GUIDE.md` ไม่ต้องแก้** — ที่นั่นพูดถึง `SEVEN_SEGMENT` ในฐานะ *ชนิดหน้าปัด*
  ซึ่งยังใช้งานปกติ ; ที่ถอนคือ *การสอบเทียบ* เท่านั้น ไม่ใช่ตัวชนิด (เกือบแก้ผิดจุด)
- `CALIBRATION-PROPOSAL.md` **ทำเครื่องหมายยกเลิก ไม่ลบทิ้ง** เพราะฉบับเก่าส่งทีม AI ไปแล้ว
  ต้องให้เขาอ่านออกว่าถอน — ลบเงียบ ๆ คือปล่อยให้เขาทำของที่เราไม่ใช้
...
## T-022: แก้คู่สีที่ตกเกณฑ์บนพื้นการ์ด + ปิดรูของสคริปต์ตรวจเอง  🟡
- ปรับ **ความสว่างอย่างเดียว คง hue เดิม** เพื่อให้หน้าตาเปลี่ยนน้อยที่สุด — ขยับแค่ 7-12 หน่วย
  ใน sRGB : `--muted`/`--dim` `#797067`→`#726961` (4.20→4.65) · `--dead` สว่าง
  `#6b7070`→`#676c6c` (4.35→4.61) · `--line` สว่าง `#8f9495`→`#83888a` (2.66→3.10) ·
  `--dead` มืด `#a89f96`→`#aba39a` (4.40→4.60)
- **หลักฐานยืนยันต้นตอ**: คอมเมนต์ของ `--line` เขียนไว้เองว่า "เดิม #8f9495 = 3.07:1 ผ่านพอดี"
  ซึ่งเป็นค่าที่วัดกับ `--bg` ทั้งที่ของพวกนี้อยู่บน `--surface` ที่เข้มกว่า
- 🔴 **ระหว่างทางพบว่าสคริปต์ที่เพิ่งเขียนเองมีรู** — ธีมมืดตั้ง `--line` เป็น `rgba()` ซึ่งไม่ใช่ hex
  แล้ว `val()` **fallback ไปวัดค่าธีมสว่างเงียบ ๆ** → รายงานว่าผ่านทั้งที่วัดผิดตัว
  ; แก้ให้ composite rgba ทับพื้นก่อนคำนวณ (สคริปต์ที่โกหกแย่กว่าไม่มีสคริปต์)
- **พอปิดรูก็เจอคู่ที่ 5 ที่ไม่เคยมีใครตรวจ**: `--line` ธีมมืด `.36` ทับพื้นการ์ดได้ `#85807b`
  = 2.93:1 ตกเกณฑ์ 3:1 → ปรับเป็น `.40` ได้ 3.26:1 ; ตรงกับ token ที่ D-014 เคยเจอว่าตกหนักสุด
