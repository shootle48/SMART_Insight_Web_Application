...
### 🚫 SEVEN_SEGMENT / WATER_METER — **ยกเลิกข้อเสนอส่วนนี้แล้ว** (2026-09-10, T-020)

> ฉบับก่อนหน้าของเอกสารนี้เคยเสนอ `bbox` + `decimals` สำหรับ `SEVEN_SEGMENT`
> **ถอนออกแล้ว ไม่ต้องทำ** — ผู้ใช้ยืนยันว่าสองชนิดนี้อ่านค่าด้วย **โมเดล** ไม่ใช่
> computer vision จึงไม่มีเรขาคณิตในภาพให้สอบเทียบ ต่างจาก GAUGE ที่ต้องรู้ว่าเข็มชี้ตรงไหน
> ถึงจะแปลงเป็นค่าได้
>
> ⇒ **`config/<point_id>` จะถูกส่งเฉพาะจุดที่เป็น `GAUGE` เท่านั้น** ; จุดชนิดอื่นไม่มี config
> ส่งลงไปเลย edge ไม่ต้องเตรียมรับ
>
> `bboxSchema` / `sevenSegmentFixtureSchema` ถูกลบออกจาก `src/contract/points.ts` แล้ว
> (ครึ่งของ D-018 ที่ว่าด้วย bbox ตกไปด้วย ; ครึ่งที่ว่าด้วยจุดอ้างอิงของ GAUGE ยังใช้อยู่)
>
> ⚠️ `SEVEN_SEGMENT` และ `WATER_METER` **ยังเป็นชนิดหน้าปัดที่ใช้งานปกติ** ในข้อความ
> `meter_frame` — ที่ถอนคือ *การสอบเทียบ* เท่านั้น ไม่ใช่ตัวชนิด
