...
1. 🔴 **push + deploy `bbc5650` ขึ้น Pi (T-023)** — หนี้ deploy กลับมาแล้วเพราะใบนี้แตะ `src/`
   ; **เป็นบั๊กที่ผู้ใช้เจอเองหน้างาน** (กราฟ 7SEG/WATER_METER ไม่ขึ้น) ยังไม่ได้แก้บนเครื่องจริง
   · **ตอน deploy ถือโอกาสดู 2 อย่างที่ยังยืนยันด้วยตาไม่ได้**: กราฟของจุด 7SEG ขึ้นจริงไหม
   และปุ่ม "บันทึก" เป็นสีส้มจริงไหมทั้งสองธีม (T-019/T-021 ตรวจได้แค่ระดับ DOM)
...
3. **T-022** คู่สี 4 คู่ตกเกณฑ์ AA บนพื้นการ์ด — `bun run check-contrast` เตือนอยู่ทุกครั้ง
