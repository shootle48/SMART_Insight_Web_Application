...
1. 🔴 **deploy 14 commit ขึ้น Pi** *(นอก roadmap แต่เร่งสุด)* — ของที่แก้ไปยังไม่เคยพิสูจน์
   บนเครื่องจริงเลย รวม bug ภาพช้า 1 เฟรมที่**ผู้ใช้เจอเองหน้างาน** ; ยิ่งดองยิ่งแพงเพราะ
   ถ้าพังจะแยกไม่ออกว่าพังเพราะ commit ไหนใน 14 ใบ → คำสั่งอยู่ที่ `docs/DEPLOYMENT.md:157`
2. **T-015 ซ้อมติดตั้งโรงงานที่ 2 บนเครื่อง dev** — ใบเดียวที่ปลดล็อก T-018 และเป็นงานที่
   ตั้งอยู่บนการเดาล้วน (ยังไม่มีใครเคยเดินเส้นนั้นจริง) ; ทำได้ทั้งที่ไปหน้างานไม่ได้
3. **T-016 / T-017** — เคาะอย่างเดียว ไม่ต้องเขียนโค้ด
4. **T-019 / T-020** — ใบเล็กจบในรอบเดียว ไว้แทรกตอนล้าหรือรออะไรอยู่

⚠️ ข้อ 1 กับข้อ 2 คนละเรื่องกัน — ข้อ 1 คือหนี้ที่ดอกเบี้ยเดินอยู่ ข้อ 2 คือแผนที่
อย่าเลือกข้อ 2 เพราะมันอยู่ในแผน ทั้งที่ข้อ 1 กำลังสะสมความเสี่ยง
...
### git — รอบนี้ยังไม่ commit
เปลี่ยนแต่เอกสาร+เครื่องมือ **ไม่ได้แตะโค้ดที่รันจริงเลยแม้แต่บรรทัดเดียว**
`M .gitignore HANDOFF.md package.json docs/{CHANGELOG,DECISIONS,TICKETS}.md`
`?? docs/ROADMAP-PACKAGE.md scripts/docs-html.ts` — 14 commit ที่ค้างรอ deploy ยังเท่าเดิม
