...
**1. ธีม/การ์ด — ทีมเอนไปทางสว่าง+กรอบใหม่แล้ว → ทำเข้า production แล้ว (2026-08-31, D-014)**
`src/web/styles.css` เปลี่ยนเป็นพื้นสว่าง + ขอบหนา + แก้ contrast 3 จุดที่ audit เจอ
(ราย ละเอียดเต็มดู D-014 ใน `docs/DECISIONS.md`) · ทดสอบผ่าน `bun run dev:all` จริงแล้ว
ครบทุก state (ok/uncertain/over/unreadable) · **ยังไม่ deploy ขึ้น Pi**
🔴 **ยังไม่ได้ยืนดูจอจริงที่โรงงาน** — เป็นแค่ "เอนไปทาง" ไม่ใช่ยืนยันสุดท้าย ตาม D-011/D-014
ที่ตกลงไว้ว่าตัดสินที่จอจริง 5 นาที ก่อน deploy ควรพาไปดูที่ Pi ก่อน หรือถามทีมยืนยันอีกที
`docs/mock/theme-mock.html` + `gauge.jpg` ยังเก็บไว้เป็นเครื่องมือเทียบ ถ้าอยากดูย้อนหลัง
(**copy ทั้งโฟลเดอร์** ไม่งั้นภาพหาย) — `scp -r docs/mock smsn@smsn-pi-office-01.local:~/mock` แล้ว `chromium ~/mock/theme-mock.html`
