...
1. 🔴 **push + deploy `a52ef95` + `bc9590e` ขึ้น Pi** — หนี้ deploy กลับมาเพราะ T-022/T-020
   แตะ `src/` · ทั้งคู่เป็นการเปลี่ยน**สีและสัญญา** ที่ต้องดูบนจอจริง:
   ตัวหนังสือรอง/ค่าเก่ายังแยกจากค่าปกติได้ด้วยตาไหม (T-022 done-when ข้อสุดท้าย)
2. 🔴 **แจ้งทีม AI ว่าถอน `bbox` ของ SEVEN_SEGMENT แล้ว** (D-022) — `CALIBRATION-PROPOSAL.md`
   ฉบับเก่าที่ส่งไปมีตัวอย่าง bbox อยู่ ; ถ้าเขาเริ่มทำฝั่ง edge รองรับไว้จะเสียเปล่า
   ⚠️ ถอนแค่ *การสอบเทียบ* — `SEVEN_SEGMENT`/`WATER_METER` ยังเป็นชนิดหน้าปัดที่ใช้ปกติ
3. 🔴 **T-027 เคาะสถาปัตยกรรม 2 ระดับ** — Pi Zero รันโมเดลไม่ไหว ทีมจะย้ายโมเดลมาไว้บน Pi 5
   ที่รัน UI อยู่ ⇒ **ล้มสมมติฐาน "ฝั่ง edge อยู่นอกขอบเขต" ของ D-020** และเปลี่ยนปลายทาง
   ของ ROADMAP-PACKAGE เป็น 2 ระดับสินค้า ; **ใบนี้ต้องมาก่อน T-028 และก่อนใบอื่นที่ตั้งอยู่
   บนขอบเขตเดิม** (T-015/T-017/T-018 อาจต้องรื้อ scope หลังเคาะ)
4. **T-024 → T-025 → T-026** ระบบแจ้งเตือนค่าเกินเกณฑ์ (design เสร็จแล้วที่ D-023)
   — ไม่ขึ้นกับ T-027 ทำคู่ขนานได้
5. **T-016 / T-017** — เคาะอย่างเดียว ไม่ต้องเขียนโค้ด

📌 **ของเก่าที่ยังเปิดค้างแต่ไม่ได้เร่ง** — T-014/T-013 รอทีม AI ฝั่ง edge · T-011 เหลือ
retention ของภาพ + `message_size_limit` · T-010 รอปลายทาง `BACKUP_REMOTE` · T-008 auth
(ตั้งใจเลื่อน ดู D-020) — อย่าเพิ่งหยิบถ้ายังไม่มีอะไรเปลี่ยน

⚠️ **browser pane ของ AI ใช้ตรวจภาพไม่ได้** — `innerWidth/innerHeight = 0` และ
`visibility: hidden` ⇒ screenshot ออกมาเป็นจอเปล่า, geometry อ่านได้ 0, `getComputedStyle`
คืนค่าค้างหลังสลับธีม ; **เชื่อได้แค่โครง DOM** เรื่องหน้าตาต้องให้คนดูเอง
