...
✅ **หนี้ deploy เคลียร์แล้ว** (`origin/main` = local = `2cc9f79` · ไม่มี commit แตะ `src/` ค้าง)
และ **ผู้ใช้ยืนยันด้วยตาบนเครื่องแล้ว 2 ข้อที่ค้างมานาน**: กราฟจุด 7SEG ขึ้นจริง (T-023) ·
ปุ่ม "บันทึก" เป็นสีส้มทั้งสองธีม (T-019/T-021) — ไม่เหลือของที่ "แก้แล้วแต่ยังไม่พิสูจน์"

1. **T-015 ซ้อมติดตั้งโรงงานที่ 2 บนเครื่อง dev** — ใบเดียวที่ปลดล็อก T-018 และเป็นงานที่
   ตั้งอยู่บนการเดาล้วน (ยังไม่มีใครเคยเดินเส้นนั้นจริงแม้แต่ครั้งเดียว) ; ทำได้ทั้งที่ไปหน้างานไม่ได้
2. **T-022** คู่สี 4 คู่ตกเกณฑ์ AA บนพื้นการ์ด — `bun run check-contrast` เตือนอยู่ทุกครั้ง
3. **T-016 / T-017** — เคาะอย่างเดียว ไม่ต้องเขียนโค้ด
4. **T-020** — ใบเล็กจบในรอบเดียว ไว้แทรกตอนล้าหรือรออะไรอยู่
